...
public class Field {
	...
	public static void generate(int size) {
		for(int i = 0; i < size; i++) {
			...
			map[0][i] = new Tile(Tile.PLAIN);
			map[i][0] = new Tile(Tile.PLAIN);
			map[size - 1][i] = new Tile(Tile.PLAIN);
			map[i][size - 1] = new Tile(Tile.PLAIN);
		}
		for(int y = 1; y < size - 1; y++) {
			for(int x = y; x < size - 1; x++) {
				...
				if(tileDeterminer <= 5 + lakeMod) {
					tile = new Tile(Tile.WATER);
				} else if(tileDeterminer <= 40 + forestMod) {
					tile = new Tile(Tile.FOREST);
				} else {
					tile = new Tile(Tile.PLAIN);
				}
